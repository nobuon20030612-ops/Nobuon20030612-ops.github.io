(function(){
  ...
  function syncRecommendUi(){
    var active=!!recommendState.active,target=active?String(recommendState.targetStat||''):'',secondary=active?String(recommendState.secondaryStat||''):'';document.querySelectorAll('[data-jinpo-recommend-stat]').forEach(function(btn){var on=active&&String(btn.getAttribute('data-jinpo-recommend-stat')||'')===target;btn.classList.toggle('active',on);btn.setAttribute('aria-pressed',on?'true':'false');});var formationSel=q('formationSelect');if(formationSel){formationSel.disabled=active;formationSel.setAttribute('aria-disabled',active?'true':'false');formationSel.classList.toggle('jinpoRecommendFormationLocked',active);var formationLabel=formationSel.closest?formationSel.closest('label'):null;if(formationLabel)formationLabel.classList.toggle('jinpoRecommendFormationLocked',active);}try{window.dispatchEvent(new CustomEvent('jinpo:recommend-state',{detail:{active:active,targetStat:target,secondaryStat:secondary,formation:String(recommendState.formation||'')}}));}catch(e){}
  }
  ...
  function ensureUiStyle(){
    ...
    var st=document.createElement('style');st.id='jinpoResultUiEnhanceStyle';st.textContent=`
      ...
      #jinpoResultSortHintSub .jinpoResultSortHintWhite{display:inline-block;vertical-align:middle;width:34px;height:14px;margin:0 4px 0 0;border:2px solid rgba(255,255,255,.92);border-radius:7px;background:linear-gradient(180deg,rgba(255,255,255,.14),rgba(58,58,58,.20));box-shadow:0 0 8px rgba(255,255,255,.18),inset 0 0 6px rgba(255,255,255,.08)}
      #jinpoResultSortHintSub .jinpoResultSortHintGrade4{color:#f2f5fb;text-shadow:0 0 8px rgba(255,255,255,.22)}
      ...
      #jinpoResultSortHintSub .jinpoResultSortHintRed{color:#e85a5a;text-shadow:0 1px 1px rgba(0,0,0,.85),0 0 3px rgba(220,35,35,.34)}
      ...
      #dbFormationList .dbPlacementMini>span.jinpoGrade4HeroName{display:inline-block !important;border:2px solid rgba(255,255,255,.92) !important;border-radius:7px !important;background:linear-gradient(180deg,rgba(255,255,255,.12),rgba(42,42,42,.18)) !important;box-shadow:0 0 10px rgba(255,255,255,.18),inset 0 0 8px rgba(255,255,255,.08) !important;outline:1px solid rgba(255,255,255,.42) !important;outline-offset:0;}
      #dbFormationList .dbPlacementMini>span.jinpoGrade4HeroName.factor4BunkyokuGlow{border-color:rgba(255,255,255,.92) !important;box-shadow:0 0 10px rgba(255,255,255,.18),inset 0 0 8px rgba(255,255,255,.08) !important;background:linear-gradient(180deg,rgba(255,255,255,.12),rgba(42,42,42,.18)) !important;}
      #dbFormationList .dbPlacementMini>span.jinpoGrade3HeroName{display:inline-block !important;border:2px solid #3f9cff !important;border-radius:7px !important;background:linear-gradient(180deg,rgba(22,78,154,.34),rgba(9,32,62,.28)) !important;box-shadow:0 0 12px rgba(63,156,255,.42),inset 0 0 8px rgba(255,255,255,.08) !important;outline:1px solid rgba(63,156,255,.82) !important;outline-offset:0;}
      #dbFormationList .dbPlacementMini>span.jinpoGrade3HeroName.factor4BunkyokuGlow{border-color:#3f9cff !important;box-shadow:0 0 12px rgba(63,156,255,.42),inset 0 0 8px rgba(255,255,255,.08) !important;background:linear-gradient(180deg,rgba(22,78,154,.34),rgba(9,32,62,.28)) !important;}
      #dbFormationList .dbMainRow>td:nth-child(2){font-size:27px !important;line-height:1 !important;font-weight:1000 !important;text-align:center !important;vertical-align:middle !important;white-space:nowrap !important;color:#ffe36e !important;text-shadow:0 0 8px rgba(255,220,72,.88),0 2px 0 rgba(72,34,0,.95) !important;font-variant-numeric:tabular-nums !important}
      #dbFormationList .dbMainRow>td:nth-child(3){font-size:22px !important;line-height:1 !important;font-weight:1000 !important;text-align:center !important;vertical-align:middle !important;white-space:nowrap !important;color:#fff0a8 !important;text-shadow:0 0 7px rgba(255,214,92,.70),0 2px 0 rgba(72,34,0,.95) !important;letter-spacing:.05em !important}
  ...
  function ensureEnhancementUi(){
    ...
    if(box&&!q('jinpoResultSummary')){
      ...
      var hintSub=document.createElement('div');hintSub.id='jinpoResultSortHintSub';hintSub.innerHTML='<span class="jinpoResultSortHintWhite" aria-hidden="true"></span><span class="jinpoResultSortHintGrade4">＝等級4</span>　/　<span class="jinpoResultSortHintBlue" aria-hidden="true"></span><span class="jinpoResultSortHintGrade3">＝等級3以下</span>　/　<span class="jinpoResultSortHintRed">赤文字＝文曲</span>';
  ...
  function sourceFor(mode,rs,own,ex){
  ...
  function lookupExactState(opts){
    opts=opts||{};var c=Number(opts.count||0),f=String(opts.formation||form()||''),mode=opts.mode||unifiedDbMode();
  ...
  async function searchRecommendedBond56(baseQuery,target,secondary,runToken){
    ...
    function buildQuery(formation,count,limit){
      return {mode:'bond56',count:count,formation:formation,ownedInternalIds:baseQuery.ownedInternalIds,excludedInternalIds:baseQuery.excludedInternalIds,rules:baseQuery.rules,priorityStats:secondary?[]:[target],sumSort:secondary?{enabled:true,stat1:target,stat2:secondary,tiePrefer:'first'}:{enabled:false,stat1:'',stat2:'',tiePrefer:'first'},factor4Max:baseQuery.factor4Max,limit:limit};
    }
  ...
  async function renderRecommended(opts){
    ...
    var rs=rules(),ownIds=ownedInternalIds(),exIds=excludedInternalIds();var query={mode:mode,targetStat:target,secondaryStat:secondary,ownedInternalIds:ownIds,excludedInternalIds:exIds,rules:rs,factor4Max:factor4Max(),limit:LIMIT};
  ...
  async function renderCurrent(opts){
    ...
    var mode=unifiedDbMode(),rs=rules(),ownIds=ownedInternalIds(),exIds=excludedInternalIds(),src=mode==='bond56'?{type:'dynamic',sortStat:''}:sourceFor(mode,rs,ownIds,exIds),query={mode:mode,count:c,formation:f,sourceType:src.type,sortStat:src.sortStat,ownedInternalIds:ownIds,excludedInternalIds:exIds,rules:rs,factor4Max:factor4Max(),limit:LIMIT};
    ...
    function acceptResult(r){
      ...
      var gradeText=mode==='grade3'?' / 等級3以下のみ':(mode==='popular'?' / 選抜人気対象のみ':(mode==='bond56'?' / 全等級 5・6因縁モード':'')),f4Text=selectedExclude>0?' / 文曲除外人数 '+selectedExclude:'';
  ...
  function ensureBond56Style(){if(q('jinpoBond56ModeStyle'))return;var st=document.createElement('style');st.id='jinpoBond56ModeStyle';st.textContent=`
    ...
    body.jinpo-bond56-mode #dbCountButtons .dbCountBtn.active{border-color:#ff43d7!important;color:#fff!important;text-shadow:0 0 6px #ff43d7!important;box-shadow:0 0 10px rgba(255,45,214,.82),0 0 18px rgba(0,237,255,.32)!important}
