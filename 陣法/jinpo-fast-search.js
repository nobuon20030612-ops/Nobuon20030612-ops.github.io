(function(){
  ...
  function renderUnifiedCountButtons(){
    ...
    box.innerHTML='<div id="grade3OnlySearchNote" class="grade3OnlySearchNote">※ON時：等級3以下のみ検索</div>'+'<button class="dbGradeToggleBtn '+(g?'active':'')+'" data-grade3-toggle="1" type="button">等級3以下 '+(g?'ON':'OFF')+'</button>'+[5,6,7,8,9].map(function(n){return '<button class="dbCountBtn '+(c===n?'active':'')+(recommendLocked?' recommendModeLocked':'')+'" data-count="'+n+'" type="button"'+(recommendLocked?' disabled aria-disabled="true" title="おすすめ陣法使用中は通常の因縁数検索を利用できません"':'')+'>'+n+'因縁</button>';}).join('')+'<div class="jinpoTenBondVisual"><img class="jinpoTenBondGuideImage" src="assets/jinpo-ten-bond-guide.png" alt="" aria-hidden="true" draggable="false"><button class="dbCountBtn jinpoTenBondPlaceholder'+(recommendLocked?' recommendModeLocked':'')+'" type="button" disabled aria-disabled="true" title="10因縁は現在利用できません">10因縁</button></div>';
