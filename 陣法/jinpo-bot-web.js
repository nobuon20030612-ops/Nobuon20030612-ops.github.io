...
(function(){
  ...
  function extractWeatherLocation(text){
    var q=stripFiller(text)
      ...
      .replace(/(?:の|で)?(?:天気予報|天気|気温|予報|降水確率|降水|湿度|風速|最高気温|最低気温).*/,'')
      .replace(/(?:は|を)?(?:教えて|知りたい|調べて|検索して)$/,'')
      .replace(/[？?！!。、\s]+$/g,'').trim();
  ...
  async function geocode(place,signal){
    var p=new URLSearchParams({name:place,count:'3',language:'ja',format:'json'});
    ...
    // 日本語サイトなので同名候補では日本を優先し、それ以外はAPI順位を尊重。
  ...
  async function fetchWeather(text,signal){
    var place=extractWeatherLocation(text);
    if(!place)return {ok:false,realtime:true,kind:'weather',needsLocation:true};
    var loc=await geocode(place,signal);
    if(!loc)return {ok:false,realtime:true,kind:'weather',notFound:true,query:place};
    var p=new URLSearchParams({
      latitude:String(loc.latitude),longitude:String(loc.longitude),timezone:'auto',forecast_days:'3',
    ...
    if(!r.ok)return {ok:false,realtime:true,kind:'weather',status:r.status};
    ...
    var lines=[];
    if(c.temperature_2m!=null)lines.push('現在 '+weatherLabel(c.weather_code)+'、'+round(c.temperature_2m,1)+'℃'+(c.apparent_temperature!=null?'（体感 '+round(c.apparent_temperature,1)+'℃）':''));
    ...
    if(Array.isArray(daily.time)&&daily.time.length){
      for(var i=0;i<Math.min(2,daily.time.length);i++){
        var day=i===0?'今日':i===1?'明日':daily.time[i],wx=Array.isArray(daily.weather_code)?daily.weather_code[i]:null,max=Array.isArray(daily.temperature_2m_max)?daily.temperature_2m_max[i]:null,min=Array.isArray(daily.temperature_2m_min)?daily.temperature_2m_min[i]:null,pop=Array.isArray(daily.precipitation_probability_max)?daily.precipitation_probability_max[i]:null;
        lines.push(day+' '+weatherLabel(wx)+(max!=null&&min!=null?'、最高 '+round(max,1)+'℃ / 最低 '+round(min,1)+'℃':'')+(pop!=null?'、降水確率 '+round(pop,0)+'%':''));
      }
    }
    return {ok:true,realtime:true,volatile:true,kind:'weather',query:place,title:where+'の天気',extract:lines.join('\n'),url:'https://open-meteo.com/',source:'Open-Meteo',sources:[{title:'Open-Meteo 天気データ',url:'https://open-meteo.com/'}],fetchedAt:Date.now(),location:{name:name,admin1:admin,country:country,latitude:loc.latitude,longitude:loc.longitude}};
  }
  ...
  window.JINPO_BOT_WEB={
    ...
    extractNewsTopic:extractNewsTopic,extractWeatherLocation:extractWeatherLocation,parseFx:parseFx,weatherLabel:weatherLabel
  };
})();
