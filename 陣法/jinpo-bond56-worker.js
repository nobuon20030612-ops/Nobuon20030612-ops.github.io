(function(){
  ...
  var heroBase=null,heroNames=null,heroInternal=null,heroFactorAvail=null,factorStride=0,bondRates=null,bondReq=null;
  ...
  function groupMax(si){var key=String(si),hit=groupMaxCache.get(key);if(hit)return hit;var out=new Uint32Array(core.gc),src=heroBase;for(var g=0;g<core.gc;g++){var best=0,off=core.gOff[g],n=core.gCnt[g];for(var j=0;j<n;j++){var h=core.mids[off+j],v=src[h*11+si];if(v>best)best=v;}out[g]=best;}groupMaxCache.set(key,out);return out;}
  function groupMin(si){var key=String(si),hit=groupMinCache.get(key);if(hit)return hit;var out=new Uint32Array(core.gc),src=heroBase;for(var g=0;g<core.gc;g++){var best=0xffffffff,off=core.gOff[g],n=core.gCnt[g];for(var j=0;j<n;j++){var h=core.mids[off+j],v=src[h*11+si];if(v<best)best=v;}out[g]=best===0xffffffff?0:best;}groupMinCache.set(key,out);return out;}
  function tripleMaskMax(si){var key=String(si),hit=tripleMaskMaxCache.get(key);if(hit)return hit;var out=new Uint32Array(core.mc),src=heroBase;for(var m=0;m<core.mc;m++){var best=0,a=core.maskOff[m],b=core.maskOff[m+1];for(var p=a;p<b;p++){var ti=core.maskOrder[p],keyv=core.tKey[ti],h1=keyv>>>18,h2=(keyv>>>9)&511,h3=keyv&511,v=src[h1*11+si]+src[h2*11+si]+src[h3*11+si];if(v>best)best=v;}out[m]=best;}tripleMaskMaxCache.set(key,out);return out;}
  function tripleMaskMin(si){var key=String(si),hit=tripleMaskMinCache.get(key);if(hit)return hit;var out=new Uint32Array(core.mc),src=heroBase;for(var m=0;m<core.mc;m++){var best=0xffffffff,a=core.maskOff[m],b=core.maskOff[m+1];for(var p=a;p<b;p++){var ti=core.maskOrder[p],keyv=core.tKey[ti],h1=keyv>>>18,h2=(keyv>>>9)&511,h3=keyv&511,v=src[h1*11+si]+src[h2*11+si]+src[h3*11+si];if(v<best)best=v;}out[m]=best===0xffffffff?0:best;}tripleMaskMinCache.set(key,out);return out;}
  function globalMax(si){var src=heroBase,best=0;for(var h=1;h<=maxHero;h++){if(!model.heroes[h])continue;var v=src[h*11+si];if(v>best)best=v;}return best;}
  function globalMin(si){var src=heroBase,best=0xffffffff;for(var h=1;h<=maxHero;h++){if(!model.heroes[h])continue;var v=src[h*11+si];if(v<best)best=v;}return best===0xffffffff?0:best;}
  ...
  function sumHeroStat(hs,si){var src=heroBase,s=0;for(var i=0;i<hs.length;i++)s+=src[hs[i]*11+si];return s;}
  function sumHeroStatMin(hs,si){var src=heroBase,s=0;for(var i=0;i<hs.length;i++)s+=src[hs[i]*11+si];return s;}
  function branchUbCycle(fixed,chosen,remainingGroups,includeGlobal,bsid,ctx){return feasibleUbPrimary(function(si){var s=sumHeroStat(fixed,si)+sumHeroStat(chosen,si);for(var k=0;k<remainingGroups.length;k++)s+=groupMax(si)[remainingGroups[k]];if(includeGlobal)s+=globalMax(si);return s;},bsid,ctx.form,ctx);}
  function branchCanMeetCycle(fixed,chosen,remainingGroups,includeGlobal,bsid,ctx){return canMeetRangeBounds(function(si){var s=sumHeroStat(fixed,si)+sumHeroStat(chosen,si);for(var k=0;k<remainingGroups.length;k++)s+=groupMax(si)[remainingGroups[k]];if(includeGlobal)s+=globalMax(si);return s;},function(si){var s=sumHeroStatMin(fixed,si)+sumHeroStatMin(chosen,si);for(var k=0;k<remainingGroups.length;k++)s+=groupMin(si)[remainingGroups[k]];if(includeGlobal)s+=globalMin(si);return s;},bsid,ctx.form,ctx);}
  function considerPlacement(p,bsid,baseSums,ctx){
    ...
    if(ctx.countOnly){
      if(ctx.f4max!==null){f4=minimalF4(p,ctx.form,bsid);if(f4===255)return;if(pop6(f4)>ctx.f4max)return;}
      if(ctx.ranges.length){var cbv=calcStats(baseSums||placementBaseSums(p),bsid,ctx.form);if(!meetsThresholds(cbv,ctx.ranges))return;}
    ...
    if(ctx.f4max!==null){f4=minimalF4(p,ctx.form,bsid);if(f4===255)return;if(pop6(f4)>ctx.f4max)return;}
    baseVals=calcStats(baseSums||placementBaseSums(p),bsid,ctx.form);if(!meetsThresholds(baseVals,ctx.ranges))return;mv=baseVals;
    ...
    var c={p:p.slice(),bsid:bsid,f4:f4,baseVals:baseVals,mv:mv,mt:probe.mt,tie:tie,semanticKey:sem};
