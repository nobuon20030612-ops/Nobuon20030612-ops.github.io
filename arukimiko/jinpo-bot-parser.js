(function(){
  ...
  function parse(input){
    ...
    if(/^(?:(?:前回|前|さっき|直前)(?:と)?同じ条件(?:で)?|(?:前回|前|さっき|直前)(?:の)?(?:検索)?条件で)\s*(?:検索して|検索|探して|探す|もう一回|もう一度)(?:やって|お願い|して)?[。！!？?\s]*$|^(?:前回|前|さっき|直前)(?:の)?検索(?:を)?(?:もう一回|もう一度)(?:やって|お願い|して)?[。！!？?\s]*$|^(?:前回|前|さっき|直前)(?:の)?検索(?:を)?(?:再実行して|やり直して|同じので)[。！!？?\s]*$/.test(t)){
    ...
    if(/^(?:それで|この条件で|今の条件で|その条件で|これで)\s*(?:検索して|検索|探して|探す)[。！!？?\s]*$|^(?:同じの|同じので|このまま|今のまま|今ので|そのまま)(?:で)?\s*(?:検索して|検索|探して|探す|もう一回|もう一度|やって|お願い)?[。！!？?\s]*$|^(?:再検索|もう一度検索|もう一回検索|検索し直して|検索やり直して)[。！!？?\s]*$/.test(t)){
