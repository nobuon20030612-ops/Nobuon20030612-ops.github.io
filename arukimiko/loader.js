/*
 * 歩き巫女 サイト共通ローダー v3.69.0-local-only
...
(function(){
  ...
  var ASSET_CACHE_VERSION=ASSET_VERSION+'-stage13ZM';
  ...
  window.ARUKIMIKO_SHARED={version:'3.69.0-local-only',baseUrl:base,pageMode:mode,loading:true,ready:false};
  ...
  function heroSavedViewHint(text,history){
    var h=Array.isArray(history)?history:[],labels=[];
    for(var i=h.length-1;i>=0&&i>=h.length-16;i--){var m=h[i]||{},d=(m.meta||{}).data||{},x=d.heroRefinement;if(m.role!=='assistant'||!d.heroKnowledge||!x||!Array.isArray(x.savedViews))continue;x.savedViews.forEach(function(v){var n=String(v&&v.name||'').trim();if(n&&labels.indexOf(n)<0)labels.push(n);});if(labels.length)break;}
    var t=normalizeKanaForRouting(text);
