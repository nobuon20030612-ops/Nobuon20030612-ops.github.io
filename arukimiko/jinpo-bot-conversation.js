...
(function(){
  ...
  function compilePersonKanaRules(rules){
    return (rules||[]).map(function(rule){
      ...
      var suffix='(?=$|[\\s、。,.!！?？「」『』（）()・]|(?:は|ハ|って|ッテ|っていう|ッテイウ|という|とゆう|と言う|の|ノ|について|ニツイテ|を|ヲ|が|ガ|も|モ|さん|サン|英傑|英欠|英決|武将|キャラ|選手|センシュ|監督|カントク|投手|トウシュ|家族|カゾク|親族|シンゾク|奥さん|オクサン|妻|ツマ|成績|セイセキ|経歴|ケイレキ|逸話|イツワ|現役|ゲンエキ|引退|インタイ|年齢|ネンレイ|何歳|ナンサイ|姓|せい|セイ|苗字|みょうじ|ミョウジ|名字|カウンター|カウンタア))';
      return {
        canonical:canonical,
  ...
  var KNOWN_INPUT_TYPO_RULES=[
    ...
    {to:'英傑',aliases:['英欠','英決','英杰','英傑つ'],fuzzy:'domain'},
  ...
  function isOpenUserFragment(text){
    ...
    // 能力名と「高い・重視・優先・検索」などの述語が揃った陣法指示は、
    // 語尾が説明調の「の」でも文として完結している。ここで一元的に完成文と判定する。
    if(/(?:生命(?:力)?|気合|腕力|耐久(?:力)?|器用(?:さ)?|知力|魅力|土(?:属性)?|水(?:属性)?|火(?:属性)?|風(?:属性)?).*(?:高い|高め|強い|強め|一番|最大|重視(?:したい)?|優先(?:したい)?|伸ばしたい|おすすめ|検索(?:して|したい)?|探して)(?:の)?$/.test(t))return false;
