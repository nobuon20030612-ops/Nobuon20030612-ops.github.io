...
(function(){
  ...
  function cleanHeroNamePatternValue(value){
    return nfkc(value)
      .replace(/^[\s　「『【"']+|[\s　」』】"']+$/g,'')
      .replace(/^(?:が|は|を|の)+|(?:の|という|っていう)$/g,'')
      .trim()
      .slice(0,24);
  }
  function heroNamePatternListCue(text){
    var t=nfkc(text);
    return /(?:すべて|全て|全部|全員|一覧|列挙|挙げて|あげて|出して|見せて|教えて|おしえて|欲しえて|教しえて|教ええて|知りたい|しりたい|いる|誰|だれ|何人|何名|何体|どんな|該当|ランキング|トップ|上位|下位|一番|最も|最高|最低|高い|低い|順)/.test(t);
  }
  function exactHeroNameExists(name){
  ...
  function heroNameFamilyGroups(){
    return Object.keys(data().families).map(function(base){return {base:base,rows:data().families[base].slice()};}).filter(function(g){return g.rows.length>=2;}).sort(function(a,b){return b.rows.length-a.rows.length||a.base.localeCompare(b.base,'ja');});
  }
  function heroVariantLabel(name){
    var m=nfkc(name).match(/\(([^()]*)\)$/);return m?S(m[1]):'';
  }
  function familyByMention(text){
    var t=nfkc(text),groups=heroNameFamilyGroups().slice().sort(function(a,b){return b.base.length-a.base.length;});
    for(var i=0;i<groups.length;i++)if(t.indexOf(groups[i].base)>=0)return groups[i];
    return null;
  }
  function familyHasExplicitVariant(text,group){
    var t=nfkc(text);return group&&group.rows.some(function(r){return heroVariantLabel(r['英傑名'])&&t.indexOf(S(r['英傑名']))>=0;});
  }
  function requestedFamilyVariant(text,group){
    if(!group)return null;
    var t=nfkc(text),rows=group.rows.slice().sort(function(a,b){return heroVariantLabel(b['英傑名']).length-heroVariantLabel(a['英傑名']).length;});
    for(var i=0;i<rows.length;i++){
      var row=rows[i],name=S(row['英傑名']),label=heroVariantLabel(name);if(!label)continue;
      if(t.indexOf(name)>=0)return row;
      var re=new RegExp('(?:^|[の\\s　、,「『【])'+esc(label)+'(?:版)?(?=(?:だけ|のみ|について|を|は|って|詳しく|教えて|見せて|知りたい|[？?。！!]|$))');
      if(re.test(t))return row;
    }
    return null;
  }
  function parseHeroFamilyOverview(text){
    var t=nfkc(text),group=familyByMention(t);
    if(group)return null;
    var generic=/(?:同じ名前|同名)(?:で|の|が|を|として|のもの)?[^。！？!?]{0,20}(?:英傑|英欠|英決|武将|キャラ)|(?:英傑|英欠|英決|武将|キャラ)[^。！？!?]{0,20}(?:同じ名前|同名)/.test(t)||
      /(?:名前|英傑名)(?:が|の)?同じ[^。！？!?]{0,16}(?:英傑|英欠|英決|武将|キャラ|もの|物|やつ)|(?:英傑|英欠|英決|武将|キャラ)[^。！？!?]{0,12}(?:名前|英傑名)(?:が|の)?同じ/.test(t)||
      /(?:別版|別衣装|別姿|派生版|別バージョン|別ばーじょん|別ver(?:sion)?)(?:が|の)?(?:ある|いる|存在する|複数ある)?[^。！？!?]{0,16}(?:英傑|英欠|英決|武将|キャラ)|(?:複数種類|複数版|複数いる|何種類もいる)[^。！？!?]{0,16}(?:英傑|英欠|英決|武将|キャラ)/i.test(t)||
      /同じ(?:英傑|英欠|英決|武将|キャラ)(?:が|で|として)?[^。！？!?]{0,10}(?:複数|何人も|何名も|何体も|何種類も)(?:いる|ある|存在する)/.test(t);
    if(!generic)return null;
    return {sameNameOverview:true,countOnly:/(?:何組|何グループ|何種類の名前)/.test(t),heroCountOnly:/(?:何人|何名|何体)(?!も)/.test(t),text:t};
  }
  function latestHeroFamilyContext(history){
    var h=Array.isArray(history)?history:[],passedAcknowledgement=false;
    for(var i=h.length-1;i>=0&&i>=h.length-20;i--){
      var m=h[i]||{},d=(m.meta&&m.meta.data)||{};if(m.role!=='assistant')continue;
      if(d.contextBoundary||d.topicSwitch||d.conversationRepair)return null;
      if(!d.heroKnowledge){
        var before=i>0?h[i-1]||{}:{},bt=nfkc(before.text||before.content||before.message||'').replace(/[。！!？?]+$/,'').trim();
        if(!passedAcknowledgement&&before.role==='user'&&/^(?:なるほど|了解|わかった|分かった|おっけー|オッケー|OK|ありがとう|ありがと)$/.test(bt)){passedAcknowledgement=true;continue;}
        return null;
      }
      if(d.nameFamily&&S(d.query))return {base:S(d.query),rows:heroNameFamilyRows(d.query)};
      if(d.nameFamilySelection&&S(d.familyBase))return {base:S(d.familyBase),rows:heroNameFamilyRows(d.familyBase)};
      if(d.familyContext&&S(d.familyBase))return {base:S(d.familyBase),rows:heroNameFamilyRows(d.familyBase)};
      if(d.hero)return null;
    }
    return null;
  }
  function parseHeroFamilyRequest(text,history){
    var t=nfkc(text),group=familyByMention(t),fromContext=false;
    // 2人以上の具体名を並べた比較は、文中の「全部比較」を同名一覧の「全部」と誤認しない。
    if(group&&exactNamedRows(t).length>=2&&/(?:比較|比べ|違い|どっち|どちら)/.test(t))return null;
    if(group&&familyHasExplicitVariant(t,group)&&!/(?:同じ名前|同名|全種類|何種類|別版|別衣装|別姿)/.test(t))return null;
    if(!group){var ctx=latestHeroFamilyContext(history);if(ctx){group={base:ctx.base,rows:ctx.rows};fromContext=true;}}
    if(!group)return null;
    var base=group.base,variant=requestedFamilyVariant(t,group),hasBase=t.indexOf(base)>=0;
    if(variant&&(!hasBase||/(?:だけ|のみ|について|詳しく|教えて|見せて|知りたい)/.test(t))){
      return {query:base,mode:'family',label:'同名',familyVariantName:S(variant['英傑名']),familyVariantLabel:heroVariantLabel(variant['英傑名']),followup:fromContext};
    }
    if(/(?:通常版|通常|無印|括弧なし|かっこなし)\s*(?:以外|を除いて|除いて|抜き)/.test(t))return {query:base,mode:'family',label:'同名',familyVariantsOnly:true,followup:fromContext};
    if(/(?:別版|別衣装|別姿|派生版)/.test(t))return {query:base,mode:'family',label:'同名',familyVariantsOnly:true,followup:fromContext};
    if(/(?:通常版|無印|括弧なし|かっこなし)/.test(t))return {query:base,mode:'family',label:'同名',familyBaseOnly:true,followup:fromContext};
    var directCue=/(?:すべて|全て|全部|全員|全種類|一覧|列挙|挙げて|あげて|出して|見せて|何人|何名|何体|何種類|種類|パターン|同名|同じ名前|各|それぞれ|ランキング|トップ|上位|下位|一番|最も|最高|最低|高い|低い|順)/.test(t);
    var sameNameCue=new RegExp(esc(base)+'(?:と|の)?同じ名前|'+esc(base)+'[^。！？!?]{0,8}同名').test(t);
    if((hasBase&&(directCue||sameNameCue))||(fromContext&&/(?:全種類|全部|全て|すべて|一覧|何人|何名|何体|何種類)/.test(t)))return {query:base,mode:'family',label:'同名',followup:fromContext};
    return null;
  }
  function renderHeroFamilyOverview(req){
    var groups=heroNameFamilyGroups(),heroCount=groups.reduce(function(sum,g){return sum+g.rows.length;},0),meta={sameNameGroupOverview:true,groupCount:groups.length,heroCount:heroCount,groups:groups.map(function(g){return {base:g.base,count:g.rows.length,heroes:g.rows.map(function(r){return r['英傑名'];})};})};
    if(req.countOnly)return result('英傑名検索','同じ基本名で複数登録されている英傑は '+groups.length+'組です。該当する登録英傑は合計 '+heroCount+'人です。',meta);
    if(req.heroCountOnly)return result('英傑名検索','同じ基本名を持つ英傑は、'+groups.length+'組・合計 '+heroCount+'人です。',meta);
    return result('英傑名検索','同じ基本名で複数登録されている英傑は '+groups.length+'組です。\n'+groups.map(function(g){return '・'+g.base+'：'+g.rows.length+'人（'+g.rows.map(function(r){return r['英傑名'];}).join(' / ')+'）';}).join('\n'),meta);
  }
  ...
  function invalidHeroNamePatternQuery(value){
    ...
    if(detectStats(q).length||detectJob(q)||detectCost(q)||detectFactors(q).length||detectSkill(q))return true;
    return /(?:因子|職業|コスト|技能|能力|ステータス|ランキング|トップ|上位|下位|高い|低い|おすすめ|配置|除外|陣法|陣形|因縁|全MAX|見聞録|鬼神石|九十九|魔導結晶|鎮魂符)/.test(q);
  }
  function parseHeroNamePattern(text){
    ...
    if(!q){
      ...
      m=t.match(/(?:英傑|英欠|英決|武将|キャラ)(?:の中)?で\s*(.{1,16}?)(?:と(?:いう|言う)|って(?:いう|言う)?)(?:名前)?(?:の)?(?:人|英傑|英欠|英決|武将|キャラ)?(?=(?:を|は|が|で|の|って|全部|全て|すべて|全員|一覧|誰|だれ|何人|何名|何体|[？?。！!]|\s|$))/);
    ...
    if(!q){
      // 「前田って名前がついてる英傑」「前田という名前が入っている武将」。
      m=t.match(/(?:^|[、。！？!?\s　])(.{1,16}?)(?:と(?:いう|言う)|って(?:いう|言う)?)(?:名前|名)(?:が)?(?:付いている|ついている|付いてる|ついてる|入っている|はいっている|入ってる|はいってる)(?:名前の)?(?:英傑|英欠|英決|武将|キャラ)/);
      set(m,'contains','名前',true);
    }
    if(!q){
      // 「前田の名前が入ってる英傑」「前田の名が付いている武将」。
      m=t.match(/(?:^|[、。！？!?\s　])(.{1,16}?)の(?:名前|名)(?:が)?(?:入っている|はいっている|入ってる|はいってる|付いている|ついている|付いてる|ついてる)(?:英傑|英欠|英決|武将|キャラ)/);
      set(m,'contains','名前',false);
    }
    if(!q){
      // 「名前に前田が入ってる英傑」。
    ...
    if(!q){
      // 「前田が名前につく英傑」。
      m=t.match(/(?:^|[、。！？!?\s　])(.{1,16}?)(?:が)?名前に(?:付く|つく|付いている|ついている|付いてる|ついてる|入る|入っている|入ってる)(?:英傑|英欠|英決|武将|キャラ)/);
      set(m,'contains','名前',false);
    }
    if(!q){
      // 「前田みたいな名前の英傑」「前田のような名前の武将」。
    ...
    if(!q&&listCue){
      ...
      m=t.match(/(?:^|[、。！？!?\s　])(.{1,16}?)(?:と(?:いう|言う)|って(?:いう|言う)?)(?:名前の)?(?:英傑|英欠|英決|武将|キャラ)(?=(?:を|は|が|の|って|で|全部|全て|すべて|全員|一覧|誰|だれ|何人|何名|何体|、|。|！|!|？|\?|\s|$))/);
      set(m,'prefix','名前',true);
    }
    if(!q&&listCue){
      m=t.match(/(?:^|[、。！？!?\s　])(.{1,16}?)(?:と名の付く|と名のつく|の名が付く|の名がつく|の名が入る|の名が入っている|の名が入ってる|の付く|のつく)(?:名前の)?(?:英傑|英欠|英決|武将|キャラ)/);
  ...
  function renderHeroNamePattern(pattern,text,priorRefinement){
    var query=S(pattern.query),allRows=heroNamePatternRows(query,pattern.mode),rows=allRows.slice(),t=nfkc(text),stats=detectStats(text),low=/低い|低め|最低|最小|ワースト|下位/.test(t),wantsRank=stats.length&&/(?:高い|低い|ランキング|トップ|上位|下位|一番|最も|最高|最低|順)/.test(t);
    function familyMeta(extra,usedRows){
      var meta={namePattern:true,nameFamily:pattern.mode==='family',query:query,matchMode:pattern.mode,namePatternLabel:pattern.label||'名前'};
      if(pattern.mode==='family'){
        meta.familyContext=true;meta.familyBase=query;meta.familyHeroes=(usedRows||rows).map(function(r){return r['英傑名'];});
        if(pattern.familyVariantsOnly)meta.familyVariantsOnly=true;
        if(pattern.familyBaseOnly)meta.familyBaseOnly=true;
      }
      return Object.assign(meta,extra||{});
    }
    if(pattern.familyVariantName){
      var selected=allRows.filter(function(r){return S(r['英傑名'])===S(pattern.familyVariantName);})[0];
      if(!selected)return result('英傑名検索','「'+query+'」の「'+S(pattern.familyVariantLabel)+'」版は、現在の英傑マスターに見つかりませんでした。',familyMeta({notFound:true,familyVariantLabel:S(pattern.familyVariantLabel)},allRows));
      return result('英傑マスター実データ',heroSummary(selected),{hero:selected['英傑名'],aspect:'summary',corrected:false,nameFamilySelection:true,familyBase:query,familyHeroes:allRows.map(function(r){return r['英傑名'];}),familyVariantLabel:S(pattern.familyVariantLabel)});
    }
    if(pattern.familyBaseOnly){
      var baseRow=allRows.filter(function(r){return S(r['英傑名'])===query;})[0];
      if(!baseRow)return result('英傑名検索','「'+query+'」には、括弧の付かない通常版は登録されていません。登録されているのは次の '+allRows.length+'人です。\n'+allRows.map(function(r){return '・'+r['英傑名'];}).join('\n'),familyMeta({familyBaseMissing:true,count:allRows.length,heroes:allRows.map(function(r){return r['英傑名'];})},allRows));
      return result('英傑マスター実データ',heroSummary(baseRow),{hero:baseRow['英傑名'],aspect:'summary',corrected:false,nameFamilySelection:true,familyBase:query,familyHeroes:allRows.map(function(r){return r['英傑名'];}),familyBaseOnly:true});
    }
    if(pattern.familyVariantsOnly)rows=allRows.filter(function(r){return S(r['英傑名'])!==query;});
    if(!rows.length){
      if(pattern.familyVariantsOnly)return result('英傑名検索','「'+query+'」には、通常版以外の別版は登録されていません。',familyMeta({familyVariantsOnly:true,count:0,heroes:[]},rows));
      return result('英傑名検索','「'+query+'」'+(pattern.mode==='contains'?'を名前に含む':'で始まる')+'英傑は見つかりませんでした。',familyMeta({notFound:true},rows));
    }
    var lead=pattern.mode==='family'?(pattern.familyVariantsOnly?'「'+query+'」の通常版以外の英傑':'「'+query+'」の同名英傑'):(pattern.label==='苗字'?'苗字が「'+query+'」の英傑':'「'+query+'」'+(pattern.mode==='contains'?'を名前に含む':'で始まる')+'英傑');
    if(pattern.excludeScope){
      ...
      return result('英傑名検索','全英傑から '+lead+' '+rows.length+'人を除くと、'+remaining.length+'人です。\n'+remainingTake.map(function(r){return '・'+r['英傑名'];}).join('\n')+(remaining.length>remainingTake.length?'\nほか '+(remaining.length-remainingTake.length)+'人います。':''),familyMeta({namePatternExclusion:true,excludedHeroes:rows.map(function(r){return r['英傑名'];}),count:remaining.length,heroes:remainingTake.map(function(r){return r['英傑名'];})},rows));
    }
    if(pattern.exhaustedFollowup)return result('英傑名検索',lead+'として登録されているのは、この '+rows.length+'人です。ほかにはいません。',familyMeta({count:rows.length,heroes:rows.map(function(r){return r['英傑名'];}),exhaustedFollowup:true},rows));
    if(wantsRank){
      ...
      var rankResult=listRanking(f,stats,text,priorRefinement);rankResult.answer=lead+'の中では、'+rankResult.answer.replace(/^全英傑の/,'');
      if(pattern.mode==='family'){rankResult.data.familyContext=true;rankResult.data.familyBase=query;rankResult.data.familyHeroes=rows.map(function(r){return r['英傑名'];});rankResult.data.nameFamily=true;rankResult.data.query=query;if(pattern.familyVariantsOnly)rankResult.data.familyVariantsOnly=true;}
      return rankResult;
    }
    if(stats.length){
      var statTake=rows.slice(0,20);
      return result('英傑名検索',lead+'は '+rows.length+'人います。登録値は次の通りです。\n'+statTake.map(function(r){return '・'+r['英傑名']+'：'+stats.map(function(st){return st+' '+r._stats[st];}).join(' / ');}).join('\n')+(rows.length>statTake.length?'\nほか '+(rows.length-statTake.length)+'人います。':'')+sourceSuffix(),familyMeta({count:rows.length,heroes:rows.map(function(r){return r['英傑名'];}),stats:stats.slice(),namePatternStats:true},rows));
    }
    if(/(?:何種類|何パターン|いくつ(?:の)?種類|種類(?:は|が)?(?:いくつ|何個))/.test(t)&&pattern.mode==='family')return result('英傑名検索',lead+'は '+rows.length+'種類です。',familyMeta({count:rows.length,heroes:rows.map(function(r){return r['英傑名'];}),familyCountKind:'種類'},rows));
    if(/何人|何名|何体|人数/.test(t))return result('英傑名検索',lead+'は '+rows.length+'人です。',familyMeta({count:rows.length,heroes:rows.map(function(r){return r['英傑名'];})},rows));
    var take=rows.slice(0,20);
    return result('英傑名検索',lead+'は '+rows.length+'人います。\n'+take.map(function(r){return '・'+r['英傑名'];}).join('\n')+(rows.length>take.length?'\nほか '+(rows.length-take.length)+'人います。':''),familyMeta({count:rows.length,heroes:rows.map(function(r){return r['英傑名'];})},rows));
  }
  ...
  function isSearchAction(text){var t=nfkc(text);return /(?:陣法|編成|組み合わせ|6人|六人|因縁|陣形|鶴翼|方円|魚鱗|衡軛|全MAX).*(?:検索|探|組)|(?:検索|適用|差替|配置|除外).*(?:して|したい|お願い)|(?:入れて|使って|含めて|固定して).*(?:探して|検索)/.test(t);}
  ...
  function derivedSaveName(text){
    var t=nfkc(text),re=/を\s*[「『【"]?([^」』】"\n]{1,20}?)[」』】"]?\s*(?:として|という名前で|という名で)\s*(?:保存|覚えて|残して)/g,m,last='';
    while((m=re.exec(t)))last=m[1];
    return cleanSavedViewName(last);
  }
  function transformedSaveName(text){
    var t=nfkc(text),patterns=[/(?:にして|に絞って|絞って|適用して|並べて|表示して|出して)\s*[「『【"]?([^」』】"\n]{1,20}?)[」』】"]?\s*(?:として|という名前で|という名で)\s*(?:保存|覚えて|残して)/,/(?:だけ|結果|候補)\s*を\s*[「『【"]?([^」』】"\n]{1,20}?)[」』】"]?\s*(?:として|という名前で|という名で)\s*(?:保存|覚えて|残して)/],m;
    for(var i=0;i<patterns.length;i++){m=t.match(patterns[i]);if(m)return cleanSavedViewName(m[1]);}
    return derivedSaveName(t);
  }
  function savedViewSetOperation(text,mentioned){
    if(!Array.isArray(mentioned)||mentioned.length<2)return null;
    var t=nfkc(text),a=mentioned[0],b=mentioned[1],an=regexEscape(a.name),bn=regexEscape(b.name);
    var directAminusB=new RegExp(an+'\\s*(?:から|より)\\s*'+bn+'\\s*(?:を)?\\s*(?:除いて|引いて|抜いて|外して|なしで|以外)');
    var directBminusA=new RegExp(bn+'\\s*(?:から|より)\\s*'+an+'\\s*(?:を)?\\s*(?:除いて|引いて|抜いて|外して|なしで|以外)');
    if(directAminusB.test(t))return {kind:'difference',a:a,b:b,label:a.name+'から'+b.name+'を除いた候補'};
    if(directBminusA.test(t))return {kind:'difference',a:b,b:a,label:b.name+'から'+a.name+'を除いた候補'};
    if(/(?:片方だけ|どちらか一方だけ|どっちか一方だけ|共通以外|重なっていない|重複していない|対称差)/.test(t))return {kind:'symmetric',a:a,b:b,label:a.name+'と'+b.name+'の片方だけにいる候補'};
    if(/(?:共通|両方にいる|どちらにもいる|どっちにもいる|重なっている|積集合)/.test(t))return {kind:'intersection',a:a,b:b,label:a.name+'と'+b.name+'の共通候補'};
    if(/(?:まとめて|合わせて|合体|統合|どちらかにいる|どっちかにいる|いずれかにいる|和集合)/.test(t))return {kind:'union',a:a,b:b,label:a.name+'と'+b.name+'をまとめた候補'};
    return null;
  }
  function hasFilterState(state){
    var s=cloneFilterState(state);return !!(s.job||s.cost||s.factors.length||s.factorSlots.length||s.skill||s.excludedJobs.length||s.excludedCosts.length||s.excludedFactors.length||s.skillPresence!==null||s.thresholds.length);
  }
  function savedViewTransformSpec(text,mentioned){
    if(!Array.isArray(mentioned)||!mentioned.length)return null;
    var t=nfkc(text),stats=detectStats(t),sample=savedViewRows(mentioned[0]),f=filteredRows(t,sample,sample.map(function(r){return r['英傑名'];})),filters=filterStateFromRowsFilter(f);
    var explicitSort=stats.length&&/(?:順|並べ|並び|ランキング|高い順|低い順|大きい順|小さい順|上から|下から|ワースト)/.test(t);
    var sortStats=explicitSort?[lastStatInText(t)||stats[stats.length-1]]:[],sortLow=explicitSort&&/(?:低い順|小さい順|下から|ワースト)/.test(t);
    var hasFilter=hasFilterState(filters),transformCue=/(?:適用|かけて|絞って|絞り込|だけ|にして|に絞|表示|出して|見せて|切り替|切替|保存|覚えて|残して|比較|比べ)/.test(t);
    if(!transformCue||(!hasFilter&&!explicitSort))return null;
    return {filters:filters,sortStats:sortStats,sortLow:sortLow,hasFilter:hasFilter,explicitSort:explicitSort};
  }
  ...
  function deltaSelectorText(text,views){
    var t=nfkc(text);(views||[]).forEach(function(v){if(v&&v.name)t=t.replace(new RegExp(regexEscape(v.name),'g'),' ');});return t;
  }
  function deltaConditionSelector(text,views){
    var t=deltaSelectorText(text,views),stats=detectStats(t),cats=[];
    function add(v){if(cats.indexOf(v)<0)cats.push(v);}
    if(/(?:職業|職種|しょくぎょう|しよくぎよう|しょくきょう|じょくぎょう|ショクギョウ)(?:条件|じょうけん|条けん|絞り込み|フィルター)?/.test(t))add('job');
    if(/(?:コスト|こすと|コスと)(?:条件|じょうけん|条けん|絞り込み|フィルター)?/.test(t))add('cost');
    if(/(?:因子|いんし|インシ)(?:条件|じょうけん|条けん|絞り込み|フィルター)?/.test(t))add('factor');
    if(/(?:固有技能|技能|ぎのう|ギノウ)(?:条件|じょうけん|条けん|絞り込み|フィルター)?/.test(t))add('skill');
    if(/(?:数値|すうち|能力値?|のうりょくち?|ステータス|すてーたす|ステ)(?:条件|じょうけん|条けん|絞り込み|フィルター)?/.test(t))add('threshold');
    if(/(?:除外条件|除外の条件|除く条件)/.test(t))add('exclusion');
    if(stats.length)add('threshold');
    var explicit=!!cats.length,except=explicit&&/(?:以外|を除いて|は除いて|抜きで|抜いて|外して)/.test(t)&&!/(?:だけ|のみ)/.test(t);
    return {explicit:explicit,categories:cats,stats:stats,except:except,text:t};
  }
  function selectDeltaFilterState(state,selector){
    var s=cloneFilterState(state);if(!selector||!selector.explicit)return cloneFilterState(s);
    var out=emptyFilterState(),cats=selector.categories||[],stats=selector.stats||[];
    function has(cat){return cats.indexOf(cat)>=0;}
    function keep(cat,stat){var hit=has(cat)||(cat==='threshold'&&stats.indexOf(stat)>=0);return selector.except?!hit:hit;}
    if(keep('job')){out.job=s.job;out.excludedJobs=s.excludedJobs.slice();}
    if(keep('cost')){out.cost=s.cost;out.excludedCosts=s.excludedCosts.slice();}
    if(keep('factor')){out.factors=s.factors.slice();out.factorSlots=s.factorSlots.map(function(x){return {slot:x.slot,factor:x.factor};});out.excludedFactors=s.excludedFactors.slice();}
    if(keep('skill')){out.skill=s.skill;out.skillPresence=s.skillPresence;}
    if(has('exclusion')){
      if(!selector.except){out.job='';out.cost=0;out.factors=[];out.factorSlots=[];out.skill='';out.skillPresence=null;out.thresholds=[];out.excludedJobs=s.excludedJobs.slice();out.excludedCosts=s.excludedCosts.slice();out.excludedFactors=s.excludedFactors.slice();}
      else{out.excludedJobs=[];out.excludedCosts=[];out.excludedFactors=[];}
    }
    out.thresholds=s.thresholds.filter(function(x){return keep('threshold',x.stat);}).map(function(x){return {stat:x.stat,value:x.value,op:x.op};});
    return out;
  }
  function inverseThresholdOp(op){if(op==='以上')return '未満';if(op==='以下')return '超える';if(/^超/.test(op))return '以下';return '以上';}
  function invertDeltaFilterState(state){
    var s=cloneFilterState(state),out=emptyFilterState(),unsupported=[];
    if(s.job)out.excludedJobs.push(s.job);
    if(s.excludedJobs.length===1)out.job=s.excludedJobs[0];else if(s.excludedJobs.length>1)unsupported.push('複数の職業除外');
    if(s.cost)out.excludedCosts.push(s.cost);
    if(s.excludedCosts.length===1)out.cost=s.excludedCosts[0];else if(s.excludedCosts.length>1)unsupported.push('複数のコスト除外');
    out.excludedFactors=s.factors.slice();out.factors=s.excludedFactors.slice();
    if(s.factorSlots.length)unsupported.push('因子位置指定');
    if(s.skill)unsupported.push('技能名の完全一致');
    if(s.skillPresence!==null)out.skillPresence=!s.skillPresence;
    out.thresholds=s.thresholds.map(function(x){return {stat:x.stat,value:x.value,op:inverseThresholdOp(x.op)};});
    return {state:out,unsupported:unsupported};
  }
  ...
  function savedViewDeltaTransferSpec(text,mentioned){
    ...
    for(var i=0;i<mentioned.length&&!source;i++)for(var j=0;j<mentioned.length&&!source;j++)for(var k=0;k<mentioned.length&&!source;k++){
      ...
      var middle='(?:条件|絞り込み|フィルター)?\\s*(?:(?:のうち|から)\\s*)?[^。？?\\n]{0,48}?';
      var p1=new RegExp(an+'\\s*(?:にあって|にはあって|に含まれていて|が持っていて)\\s*'+bn+'\\s*(?:にない|にはない|に含まれない)\\s*'+middle+cn+'\\s*(?:に|へ)');
      var p2=new RegExp(bn+'\\s*(?:にない|にはない)\\s*'+an+'\\s*の?\\s*'+middle+cn+'\\s*(?:に|へ)');
  ...
  function savedViewTransferSpec(text,mentioned){
    ...
    function set(a,b,m){source=a;target=b;mode=m;}
    for(var i=0;i<mentioned.length&&!source;i++)for(var j=0;j<mentioned.length&&!source;j++){
      if(i===j)continue;
      var a=mentioned[i],b=mentioned[j],an=regexEscape(a.name),bn=regexEscape(b.name);
      var both1=new RegExp(an+'\\s*の\\s*(?:条件|絞り込み|フィルター)\\s*(?:と|・|,|、)?\\s*(?:並び順|並び|順番)\\s*(?:を)?\\s*'+bn+'\\s*(?:に|へ)'),both2=new RegExp(bn+'\\s*(?:を|に)\\s*'+an+'\\s*(?:と)?\\s*同じ\\s*(?:条件|絞り込み|フィルター)\\s*(?:と|・|,|、)?\\s*(?:並び順|並び|順番)');
      if(both1.test(t)||both2.test(t)){set(a,b,'both');break;}
      var filter1=new RegExp(an+'\\s*の\\s*(?:条件|絞り込み条件|絞り込み|フィルター)\\s*(?:を)?\\s*'+bn+'\\s*(?:に|へ)'),filter2=new RegExp(bn+'\\s*(?:を|に)\\s*'+an+'\\s*(?:と)?\\s*同じ\\s*(?:条件|絞り込み条件|絞り込み|フィルター)'),filter3=new RegExp(an+'\\s*(?:と)?\\s*同じ\\s*(?:条件|絞り込み条件|絞り込み|フィルター)\\s*(?:を)?\\s*'+bn+'\\s*(?:に|へ)');
      if(filter1.test(t)||filter2.test(t)||filter3.test(t)){set(a,b,'filters');break;}
      var sort1=new RegExp(an+'\\s*の\\s*(?:並び順|並び|順番|ソート)\\s*(?:だけ)?\\s*(?:を)?\\s*'+bn+'\\s*(?:に|へ)'),sort2=new RegExp(bn+'\\s*(?:を|に)\\s*'+an+'\\s*(?:と)?\\s*同じ\\s*(?:並び順|並び|順番|ソート)'),sort3=new RegExp(an+'\\s*(?:と)?\\s*同じ\\s*(?:並び順|並び|順番|ソート)\\s*(?:を)?\\s*'+bn+'\\s*(?:に|へ)');
      if(sort1.test(t)||sort2.test(t)||sort3.test(t)){set(a,b,'sort');break;}
    }
    if(!source||!target||!/(?:適用|かけて|使って|反映|同じ|表示|出して|見せて|切り替|切替|保存|覚えて|残して|にして)/.test(t))return null;
    return {source:source,target:target,mode:mode,text:t,saveAs:/(?:保存|覚えて|残して)/.test(t)?transformedSaveName(t):''};
  }
  function currentSavedComparisonSpec(text,mentioned){
    if(!Array.isArray(mentioned)||mentioned.length!==1)return null;
    var t=nfkc(text),current=/(?:今|現在|この)(?:の)?(?:結果|候補|一覧|メンバー)|現在候補/.test(t),compare=/(?:比べ|比較|違い|差|共通|同じ|増え|減っ)/.test(t);
    return current&&compare?{view:mentioned[0]}:null;
  }
  function savedViewDetailSpec(text,mentioned){
    if(!Array.isArray(mentioned)||mentioned.length!==1)return null;
    var t=nfkc(text);
    if(!/(?:どんな条件|何の条件|保存時の条件|条件は|条件を教えて|何人|人数|並び順|並びは|順番|中身|内容|先頭|最初の英傑|どんな結果)/.test(t))return null;
    if(/(?:適用|かけて|使って|反映|絞って|表示して|出して|切り替|保存|削除)/.test(t))return null;
    return {view:mentioned[0]};
  }
  function parseSavedViewCommand(text,history){
    ...
    if(setOp&&/(?:表示|出して|見せて|候補に|残して|絞って|切り替|切替|使って|保存|覚えて)/.test(t)){
      setOp.action='derive';setOp.context=ctx;setOp.views=views;setOp.text=t;setOp.saveAs=/(?:保存|覚えて|残して)/.test(t)?derivedSaveName(t):'';return setOp;
    }
    if(transform&&mentioned.length>=2&&/(?:それぞれ|両方|各(?:候補|結果)?|同じ条件|比較|比べ)/.test(t))return {action:'transformCompare',context:ctx,views:views,targets:mentioned.slice(0,2),filters:transform.filters,sortStats:transform.sortStats,sortLow:transform.sortLow,text:t};
    if(transform&&mentioned.length===1)return {action:'transform',context:ctx,views:views,view:mentioned[0],filters:transform.filters,sortStats:transform.sortStats,sortLow:transform.sortLow,text:t,saveAs:/(?:保存|覚えて|残して)/.test(t)?transformedSaveName(t):''};
    if(mentioned.length>=2&&/(?:同じ条件|それぞれ.*条件|両方.*条件).*(?:比較|比べ|適用|かけ)/.test(t))return {action:'transformClarify',context:ctx,views:views};
  ...
  function renderSavedViewCommand(cmd){
    ...
    if(cmd.action==='deltaTransfer'){
      var sourceFilters=savedViewDisplayFilters(cmd.source),baselineFilters=savedViewDisplayFilters(cmd.baseline),allDeltaFilters=filterStateDelta(sourceFilters,baselineFilters),allDeltaAtoms=filterConditionAtoms(allDeltaFilters),selector=deltaConditionSelector(cmd.text,[cmd.source,cmd.baseline,cmd.target]),deltaFilters=selectDeltaFilterState(allDeltaFilters,selector),selectedAtoms=filterConditionAtoms(deltaFilters),invertRequested=/(?:反転|はんてん|反対条件|はんたい条件|逆条件|ぎゃく条件|条件を逆|逆にして)/.test(deltaSelectorText(cmd.text,[cmd.source,cmd.baseline,cmd.target])),inverted=false;
      if(!allDeltaAtoms.length)return result('英傑マスター実データ','保存結果「'+cmd.source.name+'」にあって「'+cmd.baseline.name+'」にない条件はありません。現在候補と保存内容は変更していません。'+sourceSuffix(),{savedViewCommand:true,savedViewDeltaTransfer:true,savedViewSource:cmd.source.name,savedViewBaseline:cmd.baseline.name,savedViewTarget:cmd.target.name,count:0,heroRefinement:currentData(views)});
      if(selector.explicit&&!selectedAtoms.length)return result('英傑マスター確認','指定された種類の差分条件はありません。差分条件は '+allDeltaAtoms.map(function(v){return v.label;}).join(' / ')+' です。',{needsClarification:true,savedViewCommand:true,savedViewDeltaTransfer:true,savedViewSource:cmd.source.name,savedViewBaseline:cmd.baseline.name,savedViewTarget:cmd.target.name,allDeltaConditions:allDeltaAtoms.map(function(v){return v.label;}),heroRefinement:currentData(views)});
      if(invertRequested){var inv=invertDeltaFilterState(deltaFilters);if(inv.unsupported.length)return result('英傑マスター確認','選んだ差分条件には、そのまま反対条件へ置き換えられない項目があります：'+inv.unsupported.join(' / ')+'。職業・コスト・因子・技能登録の有無・能力値の上下限なら反転できます。',{needsClarification:true,savedViewCommand:true,savedViewDeltaTransfer:true,savedViewDeltaInversion:true,unsupportedConditions:inv.unsupported.slice(),heroRefinement:currentData(views)});deltaFilters=inv.state;inverted=true;}
      var deltaAtoms=filterConditionAtoms(deltaFilters);
      if(!deltaAtoms.length)return result('英傑マスター確認','適用できる差分条件が残りませんでした。条件の種類を指定し直してください。',{needsClarification:true,savedViewCommand:true,savedViewDeltaTransfer:true,heroRefinement:currentData(views)});
      ...
      if(!rows.length)return result('英傑マスター実データ','「'+cmd.source.name+'」にあって「'+cmd.baseline.name+'」にない'+(selector.explicit?'指定条件':'条件')+(inverted?'を反対条件へ置き換えた結果':'')+'（'+deltaAtoms.map(function(v){return v.label;}).join(' / ')+'）を「'+cmd.target.name+'」へ適用すると0人でした。現在候補と保存内容は変更していません。'+sourceSuffix(),{savedViewCommand:true,savedViewDeltaTransfer:true,savedViewSource:cmd.source.name,savedViewBaseline:cmd.baseline.name,savedViewTarget:cmd.target.name,count:0,allDeltaConditions:allDeltaAtoms.map(function(v){return v.label;}),selectedDeltaConditions:selectedAtoms.map(function(v){return v.label;}),deltaConditions:deltaAtoms.map(function(v){return v.label;}),deltaConditionSelection:selector.explicit?(selector.except?'except':'only'):'all',deltaInverted:inverted,heroRefinement:currentData(views)});
      ...
      var deltaLabel=deltaAtoms.map(function(v){return v.label;}).join(' / '),shown=refinementAnswer('「'+cmd.source.name+'」にあって「'+cmd.baseline.name+'」にない'+(selector.explicit?'指定条件':'条件')+(inverted?'を反対条件へ置き換えた結果':'')+'（'+deltaLabel+'）を「'+cmd.target.name+'」へ適用'+(saved?'し、「'+saveName+'」として'+(replaced?'上書き保存':'保存'):'')+'しました。',rows,combinedFilters,sortStats,sortLow),diff=refinementDiff(ctx.activeCandidates,names);
      return result('英傑マスター実データ',shown.answer+sourceSuffix(),{savedViewCommand:true,savedViewDeltaTransfer:true,savedViewSource:cmd.source.name,savedViewBaseline:cmd.baseline.name,savedViewTarget:cmd.target.name,savedViewSaved:saved,savedViewName:saveName||'',savedViewCount:saved?names.length:undefined,count:names.length,allDeltaConditions:allDeltaAtoms.map(function(v){return v.label;}),selectedDeltaConditions:selectedAtoms.map(function(v){return v.label;}),deltaConditions:deltaAtoms.map(function(v){return v.label;}),deltaConditionSelection:selector.explicit?(selector.except?'except':'only'):'all',deltaInverted:inverted,heroes:shown.take.map(function(r){return r['英傑名'];}),candidates:targetNames.slice(),filters:cloneFilterState(combinedFilters),stats:sortStats,low:sortLow,heroRefinement:refinementData(targetNames,rows,combinedFilters,sortStats,sortLow,[],[],views),heroRefinementChange:diff,previousCandidates:diff.before,addedCandidates:diff.added,removedCandidates:diff.removed});
    }
    if(cmd.action==='detail'){
      ...
      return result('英傑マスター実データ',detailAnswer+sourceSuffix(),{savedViewCommand:true,savedViewDetail:true,savedViewName:cmd.view.name,count:detailRows.length,filters:detailFilters,stats:detailStats,low:detailLow,heroes:detailRows.slice(0,20).map(function(r){return r['英傑名'];}),heroRefinement:currentData(views)});
    }
    if(cmd.action==='compareCurrent'){
      var nowNames=cloneList(ctx.activeCandidates),savedNames=savedViewRows(cmd.view).map(function(r){return r['英傑名'];}),currentDiff=refinementDiff(nowNames,savedNames),currentCommon=nowNames.filter(function(n){return savedNames.indexOf(n)>=0;});
      var currentAnswer='現在候補と保存結果「'+cmd.view.name+'」を比較しました。\n現在候補：'+nowNames.length+'人 / '+cmd.view.name+'：'+savedNames.length+'人\n共通：'+currentCommon.length+'人'+(currentCommon.length?'（'+currentCommon.slice(0,20).join(' / ')+(currentCommon.length>20?' ほか'+(currentCommon.length-20)+'人':'')+'）':'')+'\n現在候補だけ：'+currentDiff.removed.length+'人'+(currentDiff.removed.length?'（'+currentDiff.removed.slice(0,20).join(' / ')+'）':'')+'\n'+cmd.view.name+'だけ：'+currentDiff.added.length+'人'+(currentDiff.added.length?'（'+currentDiff.added.slice(0,20).join(' / ')+'）':'');
      return result('英傑マスター実データ',currentAnswer+sourceSuffix(),{savedViewCommand:true,savedViewComparedCurrent:true,savedViewName:cmd.view.name,currentCandidates:nowNames,savedCandidates:savedNames,commonCandidates:currentCommon,onlyCurrent:currentDiff.removed,onlySaved:currentDiff.added,heroRefinement:currentData(views)});
    }
    if(cmd.action==='transfer'){
      var sourceFilters=savedViewDisplayFilters(cmd.source),sourceStats=savedViewDisplaySort(cmd.source),sourceLow=savedViewDisplaySortLow(cmd.source),useFilters=cmd.mode==='filters'||cmd.mode==='both',useSort=cmd.mode==='sort'||cmd.mode==='both';
      if(useFilters&&!hasFilterState(sourceFilters)&&!useSort)return result('英傑マスター確認','保存結果「'+cmd.source.name+'」には再利用できる絞り込み条件がありません。並び順は '+(sourceStats.length?sourceStats.join('＋')+(sourceLow?'が低い順':'が高い順'):'保存時の順番')+' です。',{needsClarification:true,savedViewCommand:true,savedViewTransfer:true,heroRefinement:currentData(views)});
      if(useSort&&!sourceStats.length&&!useFilters)return result('英傑マスター確認','保存結果「'+cmd.source.name+'」には能力の並び順が保存されていません。',{needsClarification:true,savedViewCommand:true,savedViewTransfer:true,heroRefinement:currentData(views)});
      var targetRows=savedViewRows(cmd.target),targetNames=targetRows.map(function(r){return r['英傑名'];}),transferFilters=useFilters?sourceFilters:emptyFilterState(),transferStats=useSort?sourceStats:savedViewDisplaySort(cmd.target),transferLow=useSort?sourceLow:savedViewDisplaySortLow(cmd.target),transferRows=applyFilterState(targetRows,transferFilters);
      if(transferStats.length)transferRows.sort(function(a,b){var av=scoreOf(a,transferStats),bv=scoreOf(b,transferStats),d=transferLow?av-bv:bv-av;return d||targetNames.indexOf(a['英傑名'])-targetNames.indexOf(b['英傑名']);});
      if(!transferRows.length)return result('英傑マスター実データ','「'+cmd.source.name+'」の'+(cmd.mode==='sort'?'並び順':'条件')+'を「'+cmd.target.name+'」へ適用すると0人でした。現在候補と保存内容は変更していません。'+sourceSuffix(),{savedViewCommand:true,savedViewTransfer:true,savedViewSource:cmd.source.name,savedViewTarget:cmd.target.name,count:0,heroes:[],heroRefinement:currentData(views)});
      var transferNames=transferRows.map(function(r){return r['英傑名'];}),transferSave=cleanSavedViewName(cmd.saveAs),transferRequested=/(?:保存|覚えて|残して)/.test(nfkc(cmd.text||'')),transferSaved=false,transferReplaced=false;
      if(transferRequested&&!transferSave)return result('英傑マスター確認','適用後の候補を保存する名前を教えてください。例：「BをAと同じ条件にしてCとして保存」のように指定できます。',{needsClarification:true,savedViewCommand:true,savedViewTransfer:true,heroRefinement:currentData(views)});
      if(transferSave){var transferEntry={name:transferSave,rootCandidates:transferNames.slice(),snapshot:{filters:emptyFilterState(),sortStats:transferStats.slice(),sortLow:transferLow},activeCandidates:transferNames.slice(),displayFilters:cloneFilterState(transferFilters),displaySortStats:transferStats.slice(),displaySortLow:transferLow,count:transferNames.length};transferReplaced=!!savedViewByName(views,transferSave);views=views.filter(function(v){return v.name!==transferSave;});views.push(transferEntry);views=cloneSavedViews(views);transferSaved=true;}
      var modeLabel=cmd.mode==='sort'?'並び順':(cmd.mode==='both'?'条件と並び順':'条件'),transferShown=refinementAnswer('「'+cmd.source.name+'」の'+modeLabel+'を「'+cmd.target.name+'」へ適用'+(transferSaved?'し、「'+transferSave+'」として'+(transferReplaced?'上書き保存':'保存'):'')+'しました。',transferRows,transferFilters,transferStats,transferLow),transferDiff=refinementDiff(ctx.activeCandidates,transferNames);
      return result('英傑マスター実データ',transferShown.answer+sourceSuffix(),{savedViewCommand:true,savedViewTransfer:true,savedViewTransferMode:cmd.mode,savedViewSource:cmd.source.name,savedViewTarget:cmd.target.name,savedViewSaved:transferSaved,savedViewName:transferSave||'',savedViewCount:transferSaved?transferNames.length:undefined,count:transferNames.length,heroes:transferShown.take.map(function(r){return r['英傑名'];}),candidates:targetNames.slice(),filters:cloneFilterState(transferFilters),stats:transferStats,low:transferLow,heroRefinement:refinementData(targetNames,transferRows,transferFilters,transferStats,transferLow,[],[],views),heroRefinementChange:transferDiff,previousCandidates:transferDiff.before,addedCandidates:transferDiff.added,removedCandidates:transferDiff.removed});
    }
    if(cmd.action==='transform'){
      var baseRows=savedViewRows(cmd.view),baseNames=baseRows.map(function(r){return r['英傑名'];}),rows=applyFilterState(baseRows,cmd.filters),baseSort=cloneList(cmd.view.snapshot&&cmd.view.snapshot.sortStats),sortStats=cmd.sortStats.length?cmd.sortStats.slice():baseSort,sortLow=cmd.sortStats.length?!!cmd.sortLow:!!(cmd.view.snapshot&&cmd.view.snapshot.sortLow);
      if(sortStats.length)rows.sort(function(a,b){var av=scoreOf(a,sortStats),bv=scoreOf(b,sortStats),d=sortLow?av-bv:bv-av;return d||baseNames.indexOf(a['英傑名'])-baseNames.indexOf(b['英傑名']);});
      if(!rows.length)return result('英傑マスター実データ','保存した「'+cmd.view.name+'」へ指定条件を適用すると0人でした。現在の候補と保存内容は変更していません。'+sourceSuffix(),{savedViewCommand:true,savedViewTransform:true,savedViewName:cmd.view.name,count:0,heroes:[],heroRefinement:currentData(views)});
      var saveRequested=/(?:保存|覚えて|残して)/.test(nfkc(cmd.text||'')),saveName=cleanSavedViewName(cmd.saveAs),saved=false,replaced=false,names=rows.map(function(r){return r['英傑名'];});
      if(saveRequested&&!saveName)return result('英傑マスター確認','派生した候補を保存する名前を教えてください。例：「Aを侍だけにしてCとして保存」のように指定できます。',{needsClarification:true,savedViewCommand:true,savedViewTransform:true,heroRefinement:currentData(views)});
      if(saveName){var entry={name:saveName,rootCandidates:names.slice(),snapshot:{filters:emptyFilterState(),sortStats:sortStats.slice(),sortLow:sortLow},activeCandidates:names.slice(),displayFilters:cloneFilterState(cmd.filters),displaySortStats:sortStats.slice(),displaySortLow:sortLow,count:names.length};replaced=!!savedViewByName(views,saveName);views=views.filter(function(v){return v.name!==saveName;});views.push(entry);views=cloneSavedViews(views);saved=true;}
      var label='保存した「'+cmd.view.name+'」へ '+filterStateLabel(cmd.filters)+(sortStats.length?' / '+sortStats.join('＋')+(sortLow?'が低い順':'が高い順'):'')+' を適用'+(saved?'し、「'+saveName+'」として'+(replaced?'上書き保存':'保存'):'')+'しました。',shown=refinementAnswer(label,rows,cmd.filters,sortStats,sortLow),diff=refinementDiff(ctx.activeCandidates,names);
      return result('英傑マスター実データ',shown.answer+sourceSuffix(),{savedViewCommand:true,savedViewTransform:true,savedViewSource:cmd.view.name,savedViewSaved:saved,savedViewName:saveName||'',savedViewCount:saved?names.length:undefined,count:names.length,heroes:shown.take.map(function(r){return r['英傑名'];}),candidates:baseNames.slice(),filters:cloneFilterState(cmd.filters),stats:sortStats,low:sortLow,heroRefinement:refinementData(baseNames,rows,cmd.filters,sortStats,sortLow,[],[],views),heroRefinementChange:diff,previousCandidates:diff.before,addedCandidates:diff.added,removedCandidates:diff.removed});
    }
    if(cmd.action==='transformCompare'){
      var left=cmd.targets[0],right=cmd.targets[1],applyOne=function(v){var base=savedViewRows(v),baseNames=base.map(function(r){return r['英傑名'];}),rows=applyFilterState(base,cmd.filters),stats=cmd.sortStats.length?cmd.sortStats.slice():cloneList(v.snapshot&&v.snapshot.sortStats),low=cmd.sortStats.length?!!cmd.sortLow:!!(v.snapshot&&v.snapshot.sortLow);if(stats.length)rows.sort(function(a,b){var av=scoreOf(a,stats),bv=scoreOf(b,stats),d=low?av-bv:bv-av;return d||baseNames.indexOf(a['英傑名'])-baseNames.indexOf(b['英傑名']);});return {rows:rows,names:rows.map(function(r){return r['英傑名'];}),stats:stats,low:low};},la=applyOne(left),rb=applyOne(right),diff=refinementDiff(la.names,rb.names),common=la.names.filter(function(n){return rb.names.indexOf(n)>=0;}),condition=filterStateLabel(cmd.filters),sortLabel=cmd.sortStats.length?' / '+cmd.sortStats.join('＋')+(cmd.sortLow?'が低い順':'が高い順'):'';
      var answer='保存結果「'+left.name+'」と「'+right.name+'」へ同じ条件を適用して比較しました。\n条件：'+condition+sortLabel+'\n'+left.name+'：'+la.names.length+'人'+(la.names.length?'（先頭 '+la.names.slice(0,5).join(' / ')+'）':'')+'\n'+right.name+'：'+rb.names.length+'人'+(rb.names.length?'（先頭 '+rb.names.slice(0,5).join(' / ')+'）':'')+'\n共通：'+common.length+'人'+(common.length?'（'+common.slice(0,20).join(' / ')+(common.length>20?' ほか'+(common.length-20)+'人':'')+'）':'')+'\n'+left.name+'だけ：'+diff.removed.length+'人\n'+right.name+'だけ：'+diff.added.length+'人\n元の保存結果と現在候補は変更していません。';
      return result('英傑マスター実データ',answer+sourceSuffix(),{savedViewCommand:true,savedViewTransformCompared:true,savedViewNames:[left.name,right.name],filters:cloneFilterState(cmd.filters),stats:cmd.sortStats.slice(),low:!!cmd.sortLow,leftCandidates:la.names,rightCandidates:rb.names,commonCandidates:common,onlyFirst:diff.removed,onlySecond:diff.added,heroRefinement:currentData(views)});
    }
    if(cmd.action==='derive'){
      var an=savedViewRows(cmd.a).map(function(r){return r['英傑名'];}),bn=savedViewRows(cmd.b).map(function(r){return r['英傑名'];}),am=Object.create(null),bm=Object.create(null),names=[];
      an.forEach(function(n){am[n]=1;});bn.forEach(function(n){bm[n]=1;});
      if(cmd.kind==='intersection')names=an.filter(function(n){return bm[n];});
      else if(cmd.kind==='union')names=an.concat(bn.filter(function(n){return !am[n];}));
      else if(cmd.kind==='symmetric')names=an.filter(function(n){return !bm[n];}).concat(bn.filter(function(n){return !am[n];}));
      else names=an.filter(function(n){return !bm[n];});
      if(!names.length)return result('英傑マスター実データ',cmd.label+'は0人でした。現在の候補は変更していません。'+sourceSuffix(),{savedViewCommand:true,savedViewDerived:true,savedViewOperation:cmd.kind,savedViewNames:[cmd.a.name,cmd.b.name],count:0,heroes:[],heroRefinement:currentData(views)});
      var rows=rowsByNames(names),sortStats=cloneList(cmd.a.snapshot&&cmd.a.snapshot.sortStats),sortLow=!!(cmd.a.snapshot&&cmd.a.snapshot.sortLow);
      if(sortStats.length)rows.sort(function(a,b){var av=scoreOf(a,sortStats),bv=scoreOf(b,sortStats),d=sortLow?av-bv:bv-av;return d||names.indexOf(a['英傑名'])-names.indexOf(b['英傑名']);});
      var saveName=cleanSavedViewName(cmd.saveAs),saved=false,replaced=false;
      if(/(?:保存|覚えて|残して)/.test(nfkc(cmd.text||''))&&!saveName)return result('英傑マスター確認','派生した候補を保存する名前を教えてください。例：「AとBの共通をCとして保存」のように指定できます。',{needsClarification:true,savedViewCommand:true,savedViewDerived:true,heroRefinement:currentData(views)});
      if(saveName){var entry={name:saveName,rootCandidates:names.slice(),snapshot:{filters:emptyFilterState(),sortStats:sortStats.slice(),sortLow:sortLow},activeCandidates:names.slice(),displayFilters:emptyFilterState(),displaySortStats:sortStats.slice(),displaySortLow:sortLow,count:names.length};replaced=!!savedViewByName(views,saveName);views=views.filter(function(v){return v.name!==saveName;});views.push(entry);views=cloneSavedViews(views);saved=true;}
      var diffSet=refinementDiff(ctx.activeCandidates,names),shown=refinementAnswer(cmd.label+(saved?'を「'+saveName+'」として'+(replaced?'上書き保存':'保存')+'し、現在候補にも切り替えました。':'へ切り替えました。'),rows,emptyFilterState(),sortStats,sortLow);
      return result('英傑マスター実データ',shown.answer+sourceSuffix(),{savedViewCommand:true,savedViewDerived:true,savedViewOperation:cmd.kind,savedViewNames:[cmd.a.name,cmd.b.name],savedViewSaved:saved,savedViewName:saveName||'',savedViewCount:saved?names.length:undefined,count:names.length,heroes:shown.take.map(function(r){return r['英傑名'];}),candidates:names.slice(),filters:emptyFilterState(),stats:sortStats,low:sortLow,heroRefinement:refinementData(names,rows,emptyFilterState(),sortStats,sortLow,[],[],views),heroRefinementChange:diffSet,previousCandidates:diffSet.before,addedCandidates:diffSet.added,removedCandidates:diffSet.removed});
    }
  ...
  function filterCheckRows(row,state){
    var s=cloneFilterState(state),rf=rowFactors(row),out=[];
    function add(label,pass,actual){out.push({label:label,pass:!!pass,actual:S(actual)});}
    if(s.job)add('職業「'+s.job+'」',S(row['職業'])===s.job,'登録職業：'+S(row['職業']));
    if(s.cost)add('コスト'+s.cost,Number(row['コスト'])===s.cost,'登録コスト：'+Number(row['コスト']));
    s.factors.forEach(function(f){add('因子「'+f+'」を持つ',rf.indexOf(f)>=0,'登録因子：'+(rf.join(' / ')||'なし'));});
    s.factorSlots.forEach(function(x){add('因子'+x.slot+'が「'+x.factor+'」',S(row['因子'+x.slot])===x.factor,'登録：'+(S(row['因子'+x.slot])||'なし'));});
    if(s.skill)add('技能「'+s.skill+'」',S(row['技能'])===s.skill,'登録技能：'+(S(row['技能'])||'なし'));
    s.excludedJobs.forEach(function(v){add('職業「'+v+'」を除外',S(row['職業'])!==v,'登録職業：'+S(row['職業']));});
    s.excludedCosts.forEach(function(v){add('コスト'+v+'を除外',Number(row['コスト'])!==v,'登録コスト：'+Number(row['コスト']));});
    s.excludedFactors.forEach(function(v){add('因子「'+v+'」を除外',rf.indexOf(v)<0,'登録因子：'+(rf.join(' / ')||'なし'));});
    if(s.skillPresence!==null)add(s.skillPresence?'技能登録あり':'技能登録なし',validSkill(row)===s.skillPresence,'登録技能：'+(validSkill(row)?S(row['技能']):'なし'));
    s.thresholds.forEach(function(x){var val=Number(row._stats[x.stat]||0);add(x.stat+x.value+x.op,matchesThreshold(row,[x]),x.stat+'：'+val);});
    return out;
  }
  function candidateReasonText(text){
    return nfkc(text).replace(/りゆう/g,'理由').replace(/こうほ/g,'候補').replace(/けっか/g,'結果').replace(/はいって(?:る|いる)?/g,'入っている').replace(/のこって(?:る|いる)?/g,'残っている').replace(/はずれ(?:た|てる|ている)?/g,'外れた').replace(/じょがい/g,'除外').replace(/なんで/g,'なぜ');
  }
  function candidateReasonIntent(text){
    var t=candidateReasonText(text),why=/(?:なぜ|どうして|理由|わけ)/.test(t),membership=/(?:候補|結果|一覧|この中|その中|メンバー|残って|残った|入って|含まれて|外れ|除外|消え|いない|入らない|残らない)/.test(t);
    return why&&membership;
  }
  function parseCandidateReasonQuestion(text,history){
    var t=candidateReasonText(text);if(!candidateReasonIntent(t))return null;
    var ctx=latestHeroRefinementContext(history),rows=exactNamedRows(t),fuzzy=null;
    if(rows.length>1)return {needsClarification:true,candidates:rows.map(function(r){return r['英傑名'];}),context:ctx};
    if(!rows.length){
      var m=t.match(/^(.{2,24}?)(?:が|は|って|について)?(?:候補|結果|一覧|この中|その中|メンバー|残って|残った|入って|含まれて|外れ|除外|消え|いない|入らない|残らない)/),fragment=m?m[1]:'';
      fuzzy=fragment?fuzzyRowFromFragment(fragment):fuzzyName(t);if(fuzzy&&fuzzy.row)rows=[fuzzy.row];
    }
    if(fuzzy&&fuzzy.ambiguous)return {needsClarification:true,candidates:fuzzy.candidates.slice(),query:fuzzy.query,context:ctx};
    if(!rows.length)return {needsHero:true,context:ctx};
    return {row:rows[0],correction:fuzzy&&fuzzy.corrected?{query:fuzzy.query,name:fuzzy.row['英傑名']}:null,context:ctx};
  }
  function renderCandidateReasonQuestion(q){
    if(q.needsClarification)return result('英傑マスター確認','どの英傑についての理由か確認させてください。\n'+q.candidates.join(' / '),{needsClarification:true,candidateReason:true,candidates:q.candidates.slice()});
    if(q.needsHero)return result('英傑マスター確認','候補に入った、または外れた理由を調べる英傑名を教えてください。',{needsClarification:true,candidateReason:true,needsHero:true});
    if(!q.context)return result('英傑マスター確認',q.row['英傑名']+'が入った・外れた理由を確認する候補一覧がまだありません。先に「腕力トップ10」のように候補を出してください。',{needsClarification:true,candidateReason:true,needsHeroScope:true,hero:q.row['英傑名']});
    var ctx=q.context,row=q.row,name=row['英傑名'],inRoot=ctx.rootCandidates.indexOf(name)>=0,inActive=ctx.activeCandidates.indexOf(name)>=0,checks=filterCheckRows(row,ctx.filters),passed=checks.filter(function(x){return x.pass;}),failed=checks.filter(function(x){return !x.pass;}),prefix=q.correction?'「'+q.correction.query+'」は「'+q.correction.name+'」として確認します。\n':'';
    var answer=prefix;
    if(inActive){
      answer+=name+'は現在候補に入っています。\n理由：元候補に含まれ、現在の絞り込み条件をすべて満たしています。';
      if(passed.length)answer+='\n満たしている条件：\n'+passed.map(function(x){return '・'+x.label+'（'+x.actual+'）';}).join('\n');
      else answer+='\n現在は追加の絞り込み条件がありません。';
    }else if(!inRoot){
      answer+=name+'は現在候補に入っていません。\n理由：現在の絞り込み元となる候補'+ctx.rootCandidates.length+'人に含まれていないためです。';
      if(checks.length)answer+='\nなお、現在の追加条件だけを照合すると '+passed.length+'/'+checks.length+'件を満たします。';
    }else if(failed.length){
      answer+=name+'は現在候補から外れています。\n満たしていない条件：\n'+failed.map(function(x){return '・'+x.label+'（'+x.actual+'）';}).join('\n');
      if(passed.length)answer+='\n満たしている条件：\n'+passed.map(function(x){return '・'+x.label+'（'+x.actual+'）';}).join('\n');
    }else{
      answer+=name+'は元候補に含まれ、登録値上は現在条件も満たしていますが、現在の確定候補には含まれていません。候補を保存・派生した時点の固定集合が基準になっている可能性があります。';
    }
    if(ctx.sortStats.length)answer+='\n※並び順「'+ctx.sortStats.join('＋')+(ctx.sortLow?'が低い順':'が高い順')+'」は、候補へ入るかどうかには影響しません。';
    return result('英傑マスター実データ',answer+sourceSuffix(),{candidateReason:true,hero:name,inRoot:inRoot,inCandidate:inActive,passedConditions:passed.map(function(x){return x.label;}),failedConditions:failed.map(function(x){return x.label;}),filters:cloneFilterState(ctx.filters),stats:ctx.sortStats.slice(),low:ctx.sortLow,heroRefinement:refinementData(ctx.rootCandidates,rowsByNames(ctx.activeCandidates),ctx.filters,ctx.sortStats,ctx.sortLow,ctx.undoStack,ctx.redoStack,ctx.savedViews)});
  }
  ...
  function respond(text,opt){
    opt=opt||{};
    var routedText=nfkc(text),suppliedOriginal=nfkc(opt.original||text),patternNormalized=routedText;
    try{
    ...
    if(!original||isBlocked(original)||(!savedViewCommand&&!candidateReasonQuestion&&!namePattern&&!familyOverview&&isSearchAction(original))||isNavigationOnly(original)||(!familyOverview&&!namePattern&&!hasHeroFactCue(original)&&!contextName&&!contextNames.length&&!scopeRequested&&!pairReplacement&&!groupReplacement&&!refinementEdit&&!refinementHistoryCommand&&!refinementChangeQuestion&&!savedViewCommand&&!candidateReasonQuestion))return {handled:false};
    if(savedViewCommand)return renderSavedViewCommand(savedViewCommand);
    if(familyOverview)return renderHeroFamilyOverview(familyOverview);
    if(candidateReasonQuestion)return renderCandidateReasonQuestion(candidateReasonQuestion);
    if(namePattern)return renderHeroNamePattern(namePattern,suppliedOriginal,refinementContext);
